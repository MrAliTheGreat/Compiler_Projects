...
single_statement: (oneLine_statement | assign_statement | func_call) ENDOFLINE;

func_call: ((THIS | ID) DOT)? ID LPAR func_args RPAR;

func_args: expr (COMM func_args) | expr | ;

assign_statement: var ASSIGN (NEW ID LPAR func_args RPAR | expr){System.out.println("Operator:=");};

foreachStatement: FORE{System.out.println("Loop:foreach");} LPAR (var IN (var | LBRA func_args RBRA) ) RPAR simple_scope;
...
unary_expr: UNARYBIN unary_expr{System.out.println("Operator:-");} | single_unit;

single_unit: var | {System.out.println("MethodCall");}func_call | TRUE | FALSE | STRING | NUM | LBRA func_args RBRA | LPAR expr RPAR;
...
variable: ID(LBRA expr RBRA)? DOT variable | ID(LBRA expr RBRA)?;
